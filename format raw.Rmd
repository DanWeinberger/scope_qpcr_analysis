...
e3$social_activity_type[e3$recent_social_activities=='No'] <- 'none'

e3$frequency_child_contacts[e3$contact_children=='No'] <- 'none'

e3$piab_pos <- as.factor(1*(e3$piab <40 & !is.na(e3$piab)))

e3$month <- month(e3$sample_Date)
```
...
```{r}
table1(~piab_pos|`How much contact per day do you have?` , data= e3 )

```


**Need to strandardize these categories**
```{r}
table1(~piab_pos|`If yes, what is the age range of the children you have had contact with? - Selected Choice` , data= e3 )
...
```{r}
table1(~piab_pos|`Have you taken part in any social activities or outings during the past two weeks?` , data= e3 )
...
```{r}
table1(~piab_pos|`What sorts of activities have you participated in? - Selected Choice` , data= e3 )

```
```{r}
table(e3$`What sorts of activities have you participated in? - Selected Choice`, e3$`Have you had any contact with children in the past two weeks?`)

table(e3$`What sorts of activities have you participated in? - Selected Choice`, e3$`How often do you usually have contact with children?`)
```
...
Symptoms
```{r}
table1(~piab_pos|`Which symptoms are you experiencing today? - Q13#1 - Coughing` , data= e3 )
table1(~piab_pos|`Which symptoms are you experiencing today? - Q13#1 - Running nose` , data= e3 )
table1(~piab_pos|`Which symptoms are you experiencing today? - Q13#1 - Sore throat` , data= e3 )
table1(~piab_pos|`"Which symptoms are you experiencing today? - Q13#1 - Nasal congestion` , data= e3 )

```
abx
```{r}
table1(~piab_pos|`Have you taken any antibiotics in the last two weeks?` , data= e3 )
...
Vaccine receipt (note many missing)
```{r}
table1(~piab_pos|`Have you received any new vaccines in the last two weeks? - Selected Choice` , data= e3 )
```
