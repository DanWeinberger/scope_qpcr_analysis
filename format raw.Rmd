...
q0$visitN <- '1'
q0b$visitN <- '1'
q0b$MRN <- as.numeric(gsub('MR','',q0b$MRN))
...
q0b$visitN <- as.numeric(q0b$visitN)
q2 <- bind_rows(q2, q0b[,-which(names(q0b) %in% q0.baseline.vars)])
q2 <- bind_rows(q2, q1b)
...
exp1 <- e3[,c('ID','time','piab', 'lyta','Household', 'education','ethnicity',"hh_relationship"  ,        "relationship_duration","pneu_vax_dic" ,"pneu_vax_date" ,"weight", "height","pos_covid_past" ,          "pos_covid_date","smoke_dic", "contact_children" ,       
...
baseline.chars <-unique(baseline.chars[!is.na(baseline.chars$ethnicity),])
baseline.chars <- baseline.chars %>%
  group_by(ID) %>%
  mutate(repN=row_number())
