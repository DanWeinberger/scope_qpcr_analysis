...
first_visit <- pcr_survey %>% filter(time==1 | (ID=='S1_6' & time==2)) %>%
...
flextable(t3)
```
Positivity, but considering a person to be 1 subject if re-enrolled
```{r}
...
t3 <- pcr_summary_long %>%
  ...
  mutate(common_ID= if_else(is.na(s1_match), ID, s1_match)) %>%
    filter(piab != 9999) %>%
  mutate(piab_pos = if_else(piab < 40,1,0),
         lyta_pos = if_else(lyta < 40,1,0),
         season=substr(common_ID,1,2) ) %>%
  group_by(common_ID) %>%
  summarize(any_pos_piab = max(piab_pos)) %>%
  ungroup() %>%
  summarize(N_people=n(), N_pos=sum(any_pos_piab), `Percent Positive People`=round(100*mean(any_pos_piab),1))
...
mod1.gee <- geeglm(piab_pos ~ child_contact_u12m+ child_contact_13_23m + child_contact_24_59m + child_contact_5_10y + child_contact_over10y,id=pcr_survey_mod_gee$combined_ID, data=pcr_survey_mod_gee, family=binomial(link='log'), corstr="ar1")

confint.geeglm(mod1.gee)
```
```{r}
mod1 <- geeglm(piab_pos ~ child_contact_often, data=pcr_survey_mod_gee2,  family=binomial(link='log'), corstr="ar1",id=pcr_survey_mod_gee2$combined_ID,)
...
mod1 <- geeglm(piab_pos ~ intensity_index, data=pcr_survey_mod_gee2,  family=binomial(link='log'), corstr="ar1",id=pcr_survey_mod_gee2$combined_ID,)
