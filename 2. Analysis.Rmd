...
write.csv(pcr_survey,'./Data/pcr_survey.csv')
...
write.csv(pcr_summary_long,'./Data/pcr_summary_long.csv')
...
first_visit <- pcr_survey %>% 
  group_by(combined_ID) %>%
  arrange(ID, time) %>%
  mutate(visitN_both=row_number()) %>%
  ungroup() %>%
  filter(visitN_both==1) %>%
  mutate(Race=tolower(Race), Ethnicity=tolower(Ethnicity), smoke=as.factor(smoke), pneu_vax=as.factor(pneu_vax), flu_shot=as.factor(flu_shot) , 
                  Race=if_else(Race=='unknown' |is.na(Race), 'Missing', Race),
         
         Ethnicity=if_else(Ethnicity=='unknown'|is.na(Ethnicity), 'Missing', Ethnicity),
...
flextable(tab1)
write.csv(tab1, "./Data/table1.csv")
...
```
-   Heatmap of positivity for piaB
...
- how many households? How many people?

```{r}

pcr_summary_long %>%
  filter(!is.na(Household)) %>%
  mutate(combined_HH_ID = if_else(is.na(s1_match_hh), Household, s1_match_hh)) %>%
  summarize(N_hh=length(unique(combined_HH_ID)), N_people=length(unique(combined_ID)))
...
flextable(t5) 
```
or period prevalence by sex and season
...
-   Percent positive by Pneumococcal vaccine (could be PCV or ppv23)
...
flextable(t5) 
```
an analysis of positivity by vax status? PPV status? PCV status? PPV/PCV status? Flu+PPV/PCV? Note season 2 questionaire only asked about **recent** vaccine receipt (in past 2 weeks); does not ask about whether ever vaccinated. we have permission to contact PCPs to obtain vax info but have not done this.
...
Child contact frequency vs intensity
child contact often: 0= none reported, 1=Daily; 2=every few days; 3= once or twice a month
child contact hours: 0= none reported, 1=Daily; 2=every few days; 3= once or twice a month
...
t13 <-  pcr_survey %>%
  filter(child_contact != 9999) %>% #must answer question...lots of missing surveys in S1
  mutate(child_contact_often = if_else(is.na(child_contact_often) |child_contact_often==999,0,child_contact_often) ) %>%
  group_by(child_contact_often, child_contact_hours ) %>%
  summarize(N=n()) %>%
  ungroup() %>%
  group_by(child_contact_often) %>%
  mutate(Percent= round(100*N/sum(N),2)) %>%
  filter(child_contact_often %in% c(1,2,3))

t13.t <- as_grouped_data(x = t13, groups = c("child_contact_often"), columns = NULL)
...
##   Table 2: Proportion positive, stratified by contacts
...
This makes sense--more frequent contatct, more carriage
...
Out of curiousity, break down the table of pneumo poisitivity by contact frequency AND gender. kind of interesting--me who have frequent contact have hiighest prev; but numbers are very small
...
Concentration of bacteria in those with piaB or lytA pos
...
Assumes missing survey is no contact with kids...need to clean this up a bit (e.g., if survey totally missing exclude the observation)
...
Test frequency of contact and N contacts age 1-10; each of which is associated in univariate analysis.

Generalized estimating equation to account for correlation within individuals
```{r}
pcr_survey_mod_gee <- pcr_survey_mod %>%
  mutate( 
    combined_ID = if_else(is.na(s1_match), ID, s1_match),
    combined_ID=as.factor(combined_ID),
    
    
    child_contact_1_10y = if_else(child_contact_13_23m==1 | child_contact_24_59m==1 | child_contact_5_10y==1,1,
                if_else(child_contact_13_23m==9999,9999,0)),
    child_contact_1_10y=as.factor(child_contact_1_10y),
    child_contact_u12m=as.factor(child_contact_u12m),
    child_contact_13_23m=as.factor(child_contact_13_23m),
    child_contact_24_59m=as.factor(child_contact_24_59m),
    child_contact_5_10y=as.factor(child_contact_5_10y),
    child_contact_over10y=as.factor(child_contact_over10y)
    
    #child_contact_u12m=if_else(child_contact_u12m==9999,0,child_contact_u12m),
    # child_contact_over10y =if_else(child_contact_over10y ==9999,0,child_contact_over10y )
...
test1 <- pcr_survey_mod_gee %>% group_by(combined_ID) %>% summarize(N=n()) %>% ungroup()
table(test1$N)
```

```{r}
mod1 <- geeglm(piab_pos ~ child_contact_often, 
               data=pcr_survey_mod_gee,  
               family=binomial(link='log'),
               corstr="exchangeable",id=pcr_survey_mod_gee$combined_ID,)
...
mod1.gee <- geeglm(piab_pos ~ child_contact_u12m ,id=pcr_survey_mod_gee$combined_ID, data=pcr_survey_mod_gee, family=binomial(link='log'), corstr="exchangeable")

mod2.gee <- geeglm(piab_pos ~  child_contact_13_23m,id=pcr_survey_mod_gee$combined_ID, data=pcr_survey_mod_gee, family=binomial(link='log'), corstr="exchangeable")

mod3.gee <- geeglm(piab_pos ~  child_contact_24_59m   ,id=pcr_survey_mod_gee$combined_ID, data=pcr_survey_mod_gee, family=binomial(link='log'), corstr="exchangeable")

mod4.gee <- geeglm(piab_pos ~  child_contact_5_10y ,id=pcr_survey_mod_gee$combined_ID, data=pcr_survey_mod_gee, family=binomial(link='log'), corstr="exchangeable")

mod5.gee <- geeglm(piab_pos ~  child_contact_over10y,id=pcr_survey_mod_gee$combined_ID, data=pcr_survey_mod_gee, family=binomial(link='log'), corstr="exchangeable")

# mod6.gee <- geeglm(piab_pos ~child_contact_u12m+ child_contact_1_10y,id=pcr_survey_mod_gee$combined_ID, data=pcr_survey_mod_gee, family=binomial(link='log'), corstr="exchangeable")

confint.geeglm(mod1.gee)
confint.geeglm(mod2.gee)
confint.geeglm(mod3.gee)
confint.geeglm(mod4.gee)
confint.geeglm(mod5.gee)

```
diabetes GEE
```{r}
pcr_survey_mod_gee <- pcr_survey_mod_gee %>%
  mutate(diabetes=relevel(diabetes, ref='0'))

mod6.gee <- geeglm(piab_pos ~  diabetes,id=pcr_survey_mod_gee$combined_ID, data=pcr_survey_mod_gee, family=binomial(link='log'), corstr="exchangeable")


confint.geeglm(mod6.gee)
```


```{r}

pcr_survey_mod_gee2  <- pcr_survey_mod_gee %>%
  mutate( intensity_index = if_else(child_contact_hours==3 & child_contact_often==1,1,
                            if_else(child_contact_hours==3 & child_contact_often==2,2,
                            if_else(child_contact_hours==3 & child_contact_often==3,3,
                            if_else(child_contact_hours==2 & child_contact_often==1,4,    
                            ...
                            if_else(child_contact_hours==1 & child_contact_often==2,8,
                            if_else(child_contact_hours==1 & child_contact_often==3,9,999))))))))),
          intensity_index=if_else(is.na(intensity_index),999,intensity_index),
          intensity_index=as.factor(intensity_index)) %>%
  filter(!is.na(piab_pos) & !is.na(combined_ID)) %>%
  arrange(ID, season, time)


mod1 <- geeglm(piab_pos ~ intensity_index, data=pcr_survey_mod_gee2,  family=binomial(link='log'), corstr="exchangeable",id=pcr_survey_mod_gee2$combined_ID,)
...
mod.df$child_contact_1_10 <-  if_else(mod.df$child_contact_13_23m==1 |  mod.df$child_contact_24_59m==1|mod.df$child_contact_5_10y==1,1,0)
mod.df$child_contact_1_10 =if_else(mod.df$child_contact_13_23m==9999,9999, mod.df$child_contact_1_10)
...
l2 <- l1[, c('state','child_contact_u5','child_contact_1_5','child_contact','Gender','child_contact_u12m','child_contact_13_23m',"child_contact_24_59m","child_contact_5_10y",'child_contact_over10y','child_contact_1_10', 'ID','day','time', 'season','child_contact_often','child_contact_hours', 'diabetes','piab_pos')] %>%
  ...
  mutate(diabetes=relevel(diabetes, ref='0'),
            child_contact_u10=if_else(child_contact_1_10==1|child_contact_u12m==1,1,child_contact_1_10),
    child_contact_u10=as.factor(child_contact_u10),
    
         child_contact_1_10=as.factor(child_contact_1_10),
    child_contact_u12m=as.factor(child_contact_u12m),
    child_contact_13_23m=as.factor(child_contact_13_23m),
    child_contact_24_59m=as.factor(child_contact_24_59m),
    child_contact_5_10y=as.factor(child_contact_5_10y),
...
test <- l2 %>%
  dplyr::select(state, day, ID, time,piab_pos)

msm.mod0 <- msm( state~ day ,  subject=ID , qmatrix=q1 , data=l2)
msm.mod0

msm.mod1c <- msm( state~ day ,  subject=ID , qmatrix=q1 , data=l2,covariates = list("1-2" = ~ child_contact_u10))
msm.mod1c

msm.mod1c <- msm( state~ day ,  subject=ID , qmatrix=q1 , data=l2,covariates = list("1-2" = ~ child_contact_often))
msm.mod1c
...
Frequency of child contact and acquisition and clearance rates. Those with contact with children have more frequent acquisition. Not clear if can identify acquistiion and clearance rates from these data, especially without serotype info. 
1=daily,2=every few days;3 =few times per montn
```{r}
...
or hours of contact. acquisition rate is lower for those with 4-8 and 8+ hours of reported contact--these people tend to have less frequent contacts
...
It appears those with shorter contacts have higher carriage than those with longer contacts. This could be a) confounding by age of contacts b) confoduning by frequency of contacts c) some boosting effect from frequent/intense contact. Descriptive analyses suggest it is (b)
...
msm.mod1d <- msm( state~ day ,  subject=ID , qmatrix=q1 , data=l2.cat,covariates = list("1-2" = ~ child_contact1_10_dic))
...
as a test, try same analysis using lyta as the outcome. This is problematic because we cannot distinguish lyta strains, but might give some hint.
```{r}
mod.df <- pcr_survey %>%
