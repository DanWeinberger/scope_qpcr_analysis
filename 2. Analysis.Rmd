...
```{r}
   mod1 <- glm(piab_pos ~  diabetes +pneu_vax  +child_contact_24_59m + child_contact_5_10y, family='binomial', data=pcr_survey_mod)
summary(mod1)
```
...
as a test, try same analysis using lyta as the outcome. This is problematic because we cannot distrinuish lyta strains, but might give some hint.
```{r}
mod.df <- pcr_survey %>%
